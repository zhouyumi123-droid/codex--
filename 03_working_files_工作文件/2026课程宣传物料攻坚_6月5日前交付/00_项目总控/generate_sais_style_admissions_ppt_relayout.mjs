...
const sourceScript = path.join(root, "00_项目总控/generate_sais_style_admissions_ppt_v2.mjs");
const outDir = path.join(baseDir, "PPT_横版招生手册SAIS重排版");
const pageDir = path.join(outDir, "pages");
const outPptx = path.join(outDir, "奥斯翰国际部招生宣传册_横版PPT_SAIS风格重排版.pptx");
const outMd = path.join(outDir, "奥斯翰国际部招生宣传册_横版PPT_SAIS风格重排版_说明.md");
const outVerify = path.join(outDir, "奥斯翰国际部招生宣传册_横版PPT_SAIS风格重排版_校验说明.md");
...
function roundPath(ctx, x, y, w, h, r = 18) {
  ctx.beginPath();
  ctx.moveTo(x + r, y);
  ctx.lineTo(x + w - r, y);
...
function text(ctx, str, x, y, w, size = 26, color = P.ink, lh = 1.45, weight = 400, maxLines = 99, align = "left") {
  if (currentAudit && String(str).trim()) currentAudit.push(String(str));
...
function line(ctx, x1, y1, x2, y2, color = P.line, lw = 2) {
  ctx.save();
...
function rule(ctx, x, y, w = 84, h = 4, color = P.gold) {
...
function card(ctx, x, y, w, h, title, body, color = P.gold) {
  rr(ctx, x + 12, y + 14, w, h, 18, P.shade);
  rr(ctx, x, y, w, h, 18, P.white, P.line);
  fill(ctx, color, x, y, w, 10);
  fill(ctx, P.mist, x + 1, y + 10, w - 2, 52);
  text(ctx, title, x + 28, y + 58, w - 56, 28, P.ink, 1.1, 800, 2);
  if (body) {
    const maxLines = Math.max(3, Math.floor((h - 126) / 30));
    text(ctx, body, x + 28, y + 116, w - 56, 21, P.muted, 1.45, 400, maxLines);
  }
}
function metric(ctx, num, label, x, y, c = P.red) {
  rule(ctx, x + 60, y - 18, 100, 4, c);
  text(ctx, num, x, y, 220, 66, c, 1, 800, 1, "center");
  text(ctx, label, x - 10, y + 62, 240, 22, P.ink, 1.25, 700, 2, "center");
}
function table(ctx, x, y, colWs, rowH, rows, opts = {}) {
  const totalW = colWs.reduce((a, b) => a + b, 0);
  const y0 = y;
  const bodySize = opts.bodySize || 18;
  let totalH = 0;
  rows.forEach((row, r) => {
    let xx = x;
    ...
    totalH += h;
    row.forEach((cell, c) => {
      const bg = r === 0 ? (opts.header || P.navy) : (r % 2 ? P.white : P.mist);
      fill(ctx, bg, xx, y, colWs[c], h);
      ctx.strokeStyle = P.line;
      ctx.lineWidth = 1.5;
      ctx.strokeRect(xx, y, colWs[c], h);
      const size = r === 0 ? 20 : bodySize;
      const color = r === 0 ? P.white : P.ink;
      const weight = r === 0 || (opts.firstColBold !== false && c === 0) ? 800 : 400;
      const maxLines = Math.max(2, Math.floor((h - 18) / (size * (r === 0 ? 1.18 : 1.32))));
      text(ctx, cell, xx + 14, y + (r === 0 ? 38 : 34), colWs[c] - 28, size, color, r === 0 ? 1.18 : 1.32, weight, maxLines, opts.align || "left");
      xx += colWs[c];
    });
    y += h;
  });
  ...
  ctx.lineWidth = 2;
  ctx.strokeRect(x, y0, totalW, totalH);
  return totalW;
}
async function framedImage(ctx, key, x, y, w, h, accent = P.gold, alpha = 1) {
  rr(ctx, x + 14, y + 14, w, h, 20, P.shade);
  rr(ctx, x, y, w, h, 20, P.white, P.line);
  await img(ctx, key, x + 18, y + 18, w - 36, h - 36, alpha);
  fill(ctx, accent, x + w - 24, y + 46, 8, h - 92);
}
function head(ctx, cn, en, sub = "", x = 120, y = 100, w = 1250) {
  rule(ctx, x, y + 6, 92, 4, P.gold);
  text(ctx, en.toUpperCase(), x, y + 24, w, 21, P.gold, 1, 800, 1);
  const yy = text(ctx, cn, x, y + 96, w, 60, P.ink, 1.1, 800, 2);
  line(ctx, x, yy + 20, x + Math.min(240, w - 40), yy + 20, P.lineStrong, 2);
  if (sub) return text(ctx, sub, x, yy + 64, Math.min(w, 1360), 24, P.muted, 1.45, 400, 3);
  return yy;
}
function footer(ctx, n) {
  line(ctx, 120, 1008, 1800, 1008, P.line, 1.2);
  text(ctx, "OXSTAND INTERNATIONAL SCHOOL", 120, 1037, 440, 15, "#8D97A2", 1, 700, 1);
  fill(ctx, P.gold, 1706, 1017, 94, 36);
  text(ctx, String(n).padStart(2, "0"), 1706, 1043, 94, 16, P.white, 1, 800, 1, "center");
}
async function chapter(ctx, no, cn, en, desc, imageKey = "campus1", right = true) {
  fill(ctx, P.paper, 0, 0, W, H);
  if (right) {
    fill(ctx, P.navy, 0, 0, 940, H);
    await img(ctx, imageKey, 980, 0, 940, H, 1);
    fill(ctx, "rgba(16,36,58,.18)", 980, 0, 940, H);
    fill(ctx, P.gold, 940, 0, 12, H);
    rule(ctx, 120, 146, 96, 4, P.goldSoft);
    text(ctx, no, 120, 286, 220, 96, P.goldSoft, 1, 800, 1);
    text(ctx, cn, 120, 430, 700, 72, P.white, 1.12, 800, 3);
    text(ctx, en, 125, 650, 680, 24, "#EADAB3", 1.2, 700, 2);
    line(ctx, 125, 700, 360, 700, "rgba(234,218,179,0.75)", 2);
    text(ctx, desc, 125, 774, 680, 29, "#F6EBD5", 1.45, 400, 4);
  } else {
    await img(ctx, imageKey, 0, 0, 920, H, 1);
    fill(ctx, "rgba(16,36,58,.22)", 0, 0, 920, H);
    fill(ctx, P.paper, 920, 0, 1000, H);
    fill(ctx, P.gold, 908, 0, 12, H);
    rule(ctx, 1040, 146, 96, 4, P.gold);
    text(ctx, no, 1040, 286, 220, 96, P.gold, 1, 800, 1);
    text(ctx, cn, 1040, 430, 700, 70, P.ink, 1.12, 800, 3);
    text(ctx, en, 1045, 650, 680, 24, P.gold, 1.2, 700, 2);
    line(ctx, 1045, 700, 1280, 700, P.lineStrong, 2);
    text(ctx, desc, 1045, 774, 680, 29, P.muted, 1.45, 400, 4);
  }
}
function blank() {
...
async function save(pages, name, fn) {
  ...
  pageAudit.push({ page: pages.length + 1, name, texts: currentAudit.slice() });
  currentAudit = null;
...
async function buildPages() {
  ...
  await save(pages, "cover", async (ctx) => {
    fill(ctx, P.paper, 0, 0, W, H);
    fill(ctx, P.navy, 0, 0, 990, H);
    await img(ctx, "cover", 990, 0, 930, H, 1);
    fill(ctx, "rgba(16,36,58,.16)", 990, 0, 930, H);
    fill(ctx, P.gold, 978, 0, 12, H);
    rule(ctx, 120, 152, 104, 4, P.goldSoft);
    text(ctx, "OXSTAND INTERNATIONAL SCHOOL", 120, 218, 760, 24, "#E7D4AA", 1, 800, 1);
    text(ctx, "深圳奥斯翰外语学校\n国际部招生手册", 120, 370, 760, 82, P.white, 1.14, 800, 3);
    line(ctx, 126, 624, 360, 624, "rgba(247,237,219,0.72)", 2);
    text(ctx, "精品国际课程 · 多路径升学规划 · 小规模精细化支持", 120, 700, 780, 31, "#F5E9D3", 1.4, 400, 2);
  });

  await save(pages, "contents", async (ctx) => {
    fill(ctx, P.paper, 0, 0, W, H);
    rule(ctx, 120, 118, 92, 4, P.gold);
    text(ctx, "CONTENTS", 120, 142, 520, 26, P.gold, 1, 800, 1);
    text(ctx, "目录", 120, 232, 260, 58, P.ink, 1, 800, 1);
    const items = [
    ...
    items.forEach((it, i) => {
      ...
      const x = col === 0 ? 120 : 980;
      const y = 324 + row * 132;
      rr(ctx, x + 10, y + 12, 790, 92, 18, P.shade);
      rr(ctx, x, y, 790, 92, 18, P.white, P.line);
      fill(ctx, it[3], x, y, 790, 10);
      text(ctx, it[0], x + 38, y + 62, 92, 43, it[3], 1, 800, 1);
      text(ctx, it[1], x + 150, y + 52, 460, 29, P.ink, 1, 800, 1);
      text(ctx, it[2], x + 150, y + 82, 520, 18, P.muted, 1, 400, 1);
    });
    rr(ctx, 120, 958, 1650, 62, 16, P.mist, P.line);
    text(ctx, "从学校实力到课程路径，从教学支持到入学咨询，一本册子讲清楚奥斯翰国际部的升学选择。", 150, 998, 1580, 23, P.muted, 1.3, 400, 2, "center");
  });
  ...
  await save(pages, "school-profile", async (ctx) => {
    head(ctx, "深圳本土办学二十余年", "About Oxstand", "2004年经深圳市教育局批准创办，学校位于深圳市罗湖区布心路2040号。", 120, 96, 860);
    text(ctx, "学校以“与世界同步，培育跨时代精英人才”为育人目标，运用ISO9001国际优质管理系统，植根中华传统文化，融贯东西方教育思想，依托外语特色，先后开设加拿大OSSD课程、韩国大学直升课程、日本大学直升课程、AP国际课程、IGCSE/A-Level衔接课程、新加坡IFD方向等多元升学路径。", 120, 372, 960, 27, P.ink, 1.58, 400, 8);
    const ms = [["2004", "创校时间"], ["20+", "办学积累"], ["多语种", "外语学校底色"], ["多路径", "国际升学出口"]];
    ms.forEach((m, i) => {
      const x = 120 + i * 230;
      rr(ctx, x + 8, 746, 208, 154, 18, P.shade);
      rr(ctx, x, 734, 208, 154, 18, P.white, P.line);
      metric(ctx, m[0], m[1], x - 4, 790, i % 2 ? P.teal : P.gold);
    });
    await framedImage(ctx, "cover", 1200, 130, 600, 790, P.teal, .95);
  });

  await save(pages, "advantages", async (ctx) => {
    head(ctx, "奥斯翰的四个招生优势", "Why Oxstand", "对家长而言，选择课程之前，首先要判断学校是否能稳定托举孩子三年的成长。", 120, 96, 1180);
    const items = [
    ...
    items.forEach((it, i) => {
      const x = 120 + (i % 2) * 840;
      const y = 360 + Math.floor(i / 2) * 250;
      rr(ctx, x + 14, y + 14, 760, 190, 20, P.shade);
      rr(ctx, x, y, 760, 190, 20, P.white, P.line);
      fill(ctx, it[2], x, y, 760, 10);
      text(ctx, `0${i + 1}`, x + 34, y + 68, 90, 40, it[2], 1, 800, 1);
      text(ctx, it[0], x + 150, y + 64, 520, 32, P.ink, 1.12, 800, 2);
      text(ctx, it[1], x + 150, y + 128, 540, 22, P.muted, 1.5, 400, 4);
    });
  });

  await save(pages, "history", async (ctx) => {
    head(ctx, "创校历程", "School Development History", "二十余年办学积累，形成外语特色、多元课程与国际升学服务基础。", 120, 92, 1180);
    const nodes = [
    ...
    rr(ctx, 106, 388, 1710, 392, 24, P.mist, P.line);
    const x0 = 142, y0 = 430, gapX = 202, gapY = 166;
    nodes.forEach((n, i) => {
      const x = x0 + (i % 8) * gapX;
      const y = y0 + Math.floor(i / 8) * gapY;
      if (i % 8 !== 7) line(ctx, x + 176, y + 58, x + 203, y + 58, n[2] ? P.red : P.gold, 2.4);
      rr(ctx, x + 8, y + 10, 174, 122, 14, P.shade);
      rr(ctx, x, y, 174, 122, 14, n[2] ? "#FFF8F3" : P.white, n[2] ? P.red : P.line, n[2] ? 3 : 1.5);
      fill(ctx, n[2] ? P.red : P.gold, x, y, 174, 8);
      text(ctx, n[0], x + 12, y + 50, 150, 28, n[2] ? P.red : P.gold, 1, 800, 1, "center");
      text(ctx, n[1], x + 16, y + 84, 142, 15, P.ink, 1.22, n[2] ? 800 : 400, 4, "center");
    });
    fill(ctx, P.navy, 120, 828, 1660, 94);
    text(ctx, "国际课程重点节点", 160, 887, 250, 28, P.white, 1, 800, 1);
    text(ctx, "加拿大课程、日韩小语种、UCAS资质、AP授权、IBDP世界学校等节点，构成奥斯翰多路径升学体系的基础。", 432, 884, 1280, 22, "#F4EBD8", 1.35, 400, 2);
  });
...
function pageTextsFromScript(file) {
  const source = fs.readFileSync(file, "utf8");
  const buildStart = source.indexOf("async function buildPages()");
  const buildEnd = source.indexOf("return pages;");
  const segment = buildStart >= 0 && buildEnd > buildStart ? `${source.slice(buildStart, buildEnd)}\n  return pages;` : source;
  const pages = [];
  const re = /await save\(pages,\s*"([^"]+)",[\s\S]*?(?=\n\s*await save\(pages,|\n\s*return pages;)/g;
  for (const m of segment.matchAll(re)) {
    const name = m[1];
    const body = m[0];
    const texts = [];
    for (const t of body.matchAll(/"((?:\\.|[^"\\])*)"/g)) {
      const value = t[1].replace(/\\"/g, '"').replace(/\\n/g, "\n").trim();
      if (!value) continue;
      if (/^#(?:[0-9A-Fa-f]{3,8})$/.test(value)) continue;
      if (/^rgba?\(/.test(value)) continue;
      if (/^(left|center|right)$/.test(value)) continue;
      if (/^[a-z0-9_-]+$/i.test(value) && /[a-z]/.test(value) && !/[A-Z\s]/.test(value)) continue;
      texts.push(value);
    }
    pages.push({ name, texts });
  }
  return pages;
}
...
function maskArea(ctx, x, y, w, h) {
  fill(ctx, P.paper, x, y, w, h);
}

function writeVerification() {
  const baseline = pageTextsFromScript(sourceScript);
  const current = pageTextsFromScript(new URL(import.meta.url));
  const lines = [
  ...
  if (baseline.length !== current.length) {
    ok = false;
    lines.push(`- 页数不一致：基线 ${baseline.length} 页，重排版 ${current.length} 页。`);
  } else {
    lines.push(`- 页数一致：${current.length} 页。`);
  }
  const count = Math.min(baseline.length, current.length);
  for (let i = 0; i < count; i++) {
    const a = baseline[i];
    const b = current[i];
    const ta = normalizeForCompare(a.texts);
    const tb = normalizeForCompare(b.texts);
    const sameName = a.name === b.name;
    const sameText = JSON.stringify(ta) === JSON.stringify(tb);
    if (!sameName || !sameText) {
      ok = false;
      lines.push(`- 第 ${i + 1} 页异常：基线页名=${a.name}，重排版页名=${b.name}，文本条数基线=${ta.length}，重排版=${tb.length}。`);
    }
  }
  if (ok) {
    lines.push("- 逐页页名与文本序列一致，满足“文字内容保持不变”的实现约束。");
  }
  lines.push("");
  lines.push("## 说明");
  lines.push("");
  lines.push("- 本校验覆盖脚本中以可编辑文本绘制的内容。");
  lines.push("- 图片中的内嵌文字、照片招牌等不属于本次自动文字基线对比范围。");
  fs.writeFileSync(outVerify, lines.join("\n"), "utf8");
}

const pages = await buildPages();
await makePptx(pages);
writeVerification();
fs.writeFileSync(outMd, `# 奥斯翰国际部招生宣传册横版PPT SAIS风格重排版
